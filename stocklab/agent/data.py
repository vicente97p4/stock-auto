...
class Data():
    ...
    def get_corp_code(self, name=None):
        ...
        # 예전에는 인코딩 된 api_key만 제공했으나 요즘엔 디코딩 된 api_key를 제공해서 그냥 바로 파라미터로 줘도 된다.
        res = requests.get(self.CORP_CODE_URL, params=query_params)
    ...
    def get_corp_info(self, code=None):
        ...
        # 예전에는 인코딩 된 api_key만 제공했으나 요즘엔 디코딩 된 api_key를 제공해서 그냥 바로 파라미터로 줘도 된다.
        res = requests.get(self.CORP_INFO_URL, params=query_params)
        root = ET.fromstring(res.text)
    ...
    def get_stk_distribution_info(self, code=None, date=None):
        ...
        # 예전에는 인코딩 된 api_key만 제공했으나 요즘엔 디코딩 된 api_key를 제공해서 그냥 바로 파라미터로 줘도 된다.
        res = requests.get(self.STOCK_DISTRIBUTION_URL, params=query_params)
        root = ET.fromstring(res.text)
        from_tags = root.iter("items")
        result_list = []
        for items in from_tags:
